...
PS_OUT PS_MAIN_DIRECTIONAL(PS_IN In)
{
   ...
   // float3 Li = -g_vLightDir.xyz;
    ...
    vector vColor = float4(ambientLighting, 1);
...
PS_OUT PS_MAIN_POINT(PS_IN In)
{
   ...
   // float3 Li = -g_vLightDir.xyz;
    ...
    vector vColor = float4((directLighting + ambientLighting) * fAttenuation, 1);
...
PS_OUT PS_MAIN_SPOT(PS_IN In)
{
    ...
    float3 Lo = normalize(g_vCamPosition.xyz - vWorldPos.xyz);
    float cosLo = max(0.0f, dot(vNormal.xyz, Lo));
    float3 Lradiance = g_vLightDiffuse.xyz * fAtt;
    ...
    float3 ambientLighting;
    {
        ...
        float3 irradiance = lerp(Curirradiance, Previrradiance, g_PBRLerpTime) * fAtt; // ???????????????????????????
...
float Cal_Shadow(float2 vTexcoord)
{
    ...
    if (g_isShadowSpotLight)
    {
        ...
        if (g_fSpotLightOutCutOff >= fResult) // 빛이 번질 범위 안에 있을 때
        {
            ...
            float fAtt = saturate(0.5f - ((fDistance * fDistance) / (g_fSpotLightRange * g_fSpotLightRange))); //범위 줘서 끝 범위에서는 연해지게 
            fAtt *= fAtt;
            //fAtt *= (fIntensity * fIntensity);
...
PS_OUT PS_MAIN_LIGHT_RESULT(PS_IN In)
{
    PS_OUT Out = (PS_OUT) 0;

    vector vColor = g_DiffuseTexture.Sample(LinearSamplerClamp, In.vTexcoord);
    if (0.0f == vColor.a)
        discard;
    ...
    if (g_isSSAO)
    {
        Out.vColor *= g_SSAOTexture.Sample(LinearSamplerClamp, In.vTexcoord);
    }
    
    
    float fShadow = Cal_Shadow(In.vTexcoord);
    Out.vColor *= (1 - fShadow);
    Out.vColor.a = 1;
