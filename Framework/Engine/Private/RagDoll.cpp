...
PxRigidDynamic* CRagdoll::find_recent_body_Cloth(uint32_t idx, Skeleton* skeleton, uint32_t& chosen_idx)
{
    Joint* joints = skeleton->joints();

    PxRigidDynamic* body;

    if (m_rigid_bodies_BreakPart_Cloth[idx] == nullptr)
    {
        chosen_idx = idx;

        idx = joints[idx].parent_index_BreakPart_Cloth;
        body = m_rigid_bodies[idx];
        chosen_idx = idx;
    }
    else
    {
        body = m_rigid_bodies_BreakPart_Cloth[idx];
    }
