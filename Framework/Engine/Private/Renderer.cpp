...
{
	/* MRT_GameObjects : °´Ã¼µéÀÇ Æ¯Á¤ Á¤º¸¸¦ ¹Þ¾Æ¿À±âÀ§ÇÑ ·»´õÅ¸°ÙµéÀÌ´Ù. */
	...
	/* MRT_LightAcc : ºûµéÀÇ ¿¬»ê°á°ú Á¤º¸¸¦ ¹Þ¾Æ¿À±âÀ§ÇÑ ·»´õÅ¸°ÙµéÀÌ´Ù. */
	if (FAILED(m_pGameInstance->Add_MRT(TEXT("MRT_LightAcc"), TEXT("Target_Shade"))))
		return E_FAIL;
	if (FAILED(m_pGameInstance->Add_MRT(TEXT("MRT_LightAcc"), TEXT("Target_Specular"))))
		return E_FAIL;

	/* MRT_Shadow_Spot : °´Ã¼µéÀÇ Æ¯Á¤ Á¤º¸¸¦ ¹Þ¾Æ¿À±âÀ§ÇÑ ·»´õÅ¸°ÙµéÀÌ´Ù. */
	...
	/* MRT_LightResult : ºû¿¬»ê±îÁö È¥ÇÕÇÑ ±×¸²À» ¹öÆÛ·Î ÀúÀå */
	...
	/* MRT_Ambient : Æ÷½ºÆ® ÇÁ·Î¼¼½Ì¿¡ ÀÌ¿ëµÉ È¯°æ »ö */
	...
	/* MRT_PostProcessing : Æ÷½ºÆ® ÇÁ·Î¼¼½Ì */
...
HRESULT CRenderer::SetUp_LightDSV_Point()
{
	...
	TextureDesc.MipLevels = 1;
	TextureDesc.ArraySize = m_iArraySize * 6;							// ÃÖ´ë ±íÀÌ ¹öÆÛ °³¼ö : ±¤¿ø °³¼ö¿¡ µû¶ó ´Ù¸§ ¿©±â¼­´Â 4, Å¥ºê ¸ÊÀÇ °æ¿ì 6ÀÇ ¹è¼ö·Î ÁöÁ¤
	TextureDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
...
{
	/* µðÆÛµå ½¦ÀÌµùÀ» À§ÇÑ ·»´õÅ¸°ÙµéÀ» »ý¼ºÇÏÀÚ. */
	...
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Depth"), static_cast<_uint>(ViewportDesc.Width), static_cast<_uint>(ViewportDesc.Height), DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.f, 1.f, 0.f, 1.f))))
		return E_FAIL;
	/* For.Target_Material */			//	±Ý¼Ó¼º, °ÅÄ¥±â, ¹Ý»çÀ², ÀÌ¹Ì½Ãºê
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Material"), static_cast<_uint>(ViewportDesc.Width), static_cast<_uint>(ViewportDesc.Height), DXGI_FORMAT_R8G8B8A8_UNORM, _float4(0.f, 0.f, 0.f, 0.f))))
		return E_FAIL;
	/* For.Target_Origin */			//	ÀÚ±â ±×¸²ÀÚ ¹æÁö¸¦ À§ÇÑ ·»´õ Å¸°Ù
...
HRESULT CRenderer::SetUp_RenderTargets_Pre_PostProcessing(const D3D11_VIEWPORT& ViewportDesc)
{
	...
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Pre_Post_Depth"), static_cast<_uint>(ViewportDesc.Width), static_cast<_uint>(ViewportDesc.Height), DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.f, 1.f, 0.f, 1.f))))
		return E_FAIL;
	/* For.Target_Pre_Post_Material */			//	±Ý¼Ó¼º, °ÅÄ¥±â, ¹Ý»çÀ², Ãß°¡ ÇÏ³ª´õ °¡´É
...
<<<<<<< HEAD
	/* ¹é¹öÆÛ¿¡´Ù°¡ µðÆÛµå ¹æ½ÄÀ¸·Î ¿¬»êµÈ ÃÖÁ¾ °á°ú¹°À» Âï¾îÁØ´Ù. */
...
{
	/* ·»´õÅ¸°ÙÀ» ±³Ã¼ÇÑ´Ù. */
	/* ÀÌ ±×·ì¿¡ ÀÖ´Â °´Ã¼µéÀ» ´Ù ºû¿¬»êÀÌ ÇÊ¿äÇÏ´Ù. => ºû¿¬»êÀ» ÈÄÃ³¸®·Î ÇÒ²¨´Ù. */
	/* ÈÄÃ³¸®¸¦ À§ÇØ¼­´Â ºû¿¬»êÀ» À§ÇÑ µ¥ÀÌÅÍ°¡ ÇÊ¿äÇÏ´Ù. => ºû : ºû¸Å´ÏÁ®, ¡Ù³ë¸Ö,ÀçÁú¡Ù : ÀÌ»õ³¢¸¦ ¹Þ¾Æ¿À°í ½Í¾î¼­!!!!! ·»´õÅ¸°Ù¿¡ ¤¸2¤ÃÀåÇØ¼­ ¹Þ¾Æ¿Ã¶ó°í!! */
	/* Diffuse¸¦ 0¹øÂ°¿¡ ¼Â, Normal¸¦ 1¹øÂ°¿¡ ¼Â */
