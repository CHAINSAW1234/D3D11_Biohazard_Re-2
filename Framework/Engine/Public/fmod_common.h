...
typedef struct FMOD_VECTOR
{
    ...
    FMOD_VECTOR(const FMOD_VECTOR&) = default;
    FMOD_VECTOR& operator=(const FMOD_VECTOR&) = default;

    FMOD_VECTOR(FMOD_VECTOR&&) = default;
    FMOD_VECTOR& operator=(FMOD_VECTOR&&) = default;

    FMOD_VECTOR(XMFLOAT3 vXMFloat3): x(vXMFloat3.x), y(vXMFloat3.y), z(vXMFloat3.z) {}
    FMOD_VECTOR& operator=(XMFLOAT3&& vXMFloat3) { x = vXMFloat3.x; y = vXMFloat3.y; z = vXMFloat3.z; return *this; }

    FMOD_VECTOR(XMFLOAT4 vXMFloat4) : x(vXMFloat4.x), y(vXMFloat4.y), z(vXMFloat4.z) {}
    FMOD_VECTOR& operator=(XMFLOAT4&& vXMFloat4) { x = vXMFloat4.x; y = vXMFloat4.y; z = vXMFloat4.z; return *this;}
    
    constexpr FMOD_VECTOR(float _x, float _y, float _z) noexcept : x(_x), y(_y), z(_z) {}
    explicit FMOD_VECTOR(_In_reads_(3) const float* pArray) noexcept : x(pArray[0]), y(pArray[1]), z(pArray[2]) {}
