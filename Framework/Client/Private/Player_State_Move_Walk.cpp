...
#pragma region 키 입력에 따른 애니메이션 변경
		if (m_dwDirection & DIRECTION_FRONT) {
			...
			m_pPlayer->Get_Body_Model()->Set_BlendWeight(0, 0.5f, 10.f);
			m_pPlayer->Get_Body_Model()->Set_BlendWeight(1, 0.5f, 10.f);
...
void CPlayer_State_Move_Walk::Update_Sound()
{
	...
	if (!isPlayed && iTrackPosition == 24) {
		isPlayed = true;	
		m_pPlayer->Change_Sound_3D(TEXT("Sound_Player_FootStep_Wood"), 9, 2);
	}

	if (isPlayed && iTrackPosition == 0) {
		isPlayed = false;
	}
