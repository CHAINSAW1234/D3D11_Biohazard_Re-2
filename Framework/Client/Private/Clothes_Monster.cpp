...
CClothes_Monster::CClothes_Monster(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPartObject{ pDevice, pContext }
{
}

CClothes_Monster::CClothes_Monster(const CClothes_Monster& rhs)
	: CPartObject{ rhs }
{
}

HRESULT CClothes_Monster::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CClothes_Monster::Initialize(void* pArg)
{
	if (nullptr == pArg)
		return E_FAIL;
...
void CClothes_Monster::Priority_Tick(_float fTimeDelta)
{
	__super::Priority_Tick(fTimeDelta);
}

void CClothes_Monster::Tick(_float fTimeDelta)
{
	__super::Tick(fTimeDelta);
}

void CClothes_Monster::Late_Tick(_float fTimeDelta)
{
...
HRESULT CClothes_Monster::Render_LightDepth_Dir()
{
...
HRESULT CClothes_Monster::Render_LightDepth_Point()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
HRESULT CClothes_Monster::Render_LightDepth_Spot()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
HRESULT CClothes_Monster::Initialize_Model()
{
...
HRESULT CClothes_Monster::Bind_ShaderResources()
{
	if (m_bRagdoll)
	{
		auto WorldMat = m_pParentsTransform->Get_WorldFloat4x4();
...
CClothes_Monster* CClothes_Monster::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CClothes_Monster* pInstance = { new CClothes_Monster(pDevice, pContext) };

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CClothes_Monster"));
...
CGameObject* CClothes_Monster::Clone(void* pArg)
{
	CClothes_Monster* pInstance = { new CClothes_Monster(*this) };
