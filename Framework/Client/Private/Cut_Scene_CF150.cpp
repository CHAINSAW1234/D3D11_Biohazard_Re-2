...
CCut_Scene_CF150::CCut_Scene_CF150(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CCut_Scene{ pDevice, pContext }
{
}

CCut_Scene_CF150::CCut_Scene_CF150(const CCut_Scene_CF150& rhs)
	: CCut_Scene{ rhs }
{
}

HRESULT CCut_Scene_CF150::Initialize_Prototype()
{
...
HRESULT CCut_Scene_CF150::Initialize(void* pArg)
{
	if (FAILED(__super::Initialize(pArg)))
		return E_FAIL;

	//	m_pGameInstance->Add_Object_Sound(m_pTransformCom, 11);
	CCut_Scene_Manager::Get_Instance()->Add_CutScene(CCut_Scene_Manager::CF_ID::_150, this);

	return S_OK;
}

void CCut_Scene_CF150::Priority_Tick(_float fTimeDelta)
{
...
void CCut_Scene_CF150::Tick(_float fTimeDelta)
{
	__super::Tick(fTimeDelta);
}

void CCut_Scene_CF150::Late_Tick(_float fTimeDelta)
{
	__super::Late_Tick(fTimeDelta);
}

HRESULT CCut_Scene_CF150::SetUp_Animation_Layer()
{
	////	For.PL0000
	//if (FAILED(m_Actors[static_cast<_uint>(CF150_ACTOR_TYPE::_PL_0000)]->Set_Animation(static_cast<_uint>(CActor_PL00::ACTOR_PL00_PART::_BODY), TEXT("CF150_PL0000"), 0)))
	//	return E_FAIL;

	////	For.SM41_024
	//if (FAILED(m_Actors[static_cast<_uint>(CF150_ACTOR_TYPE::_SM41_024)]->Set_Animation(static_cast<_uint>(CActor_SM41_024::ACTOR_SM41_024_PART::_BODY), TEXT("CF150_SM41_024"), 0)))
	//	return E_FAIL;

	return S_OK;
}

void CCut_Scene_CF150::Start_CutScene()
{
	...
	/*CProp_Controller* pProp_Controller = { m_PropControllers[static_cast<_uint>(CF150_PROP_TYPE::_SM41_024)] };
...
void CCut_Scene_CF150::Finish_CutScene()
{
	...
	//	m_Actors[static_cast<_uint>(CF150_ACTOR_TYPE::_PL_0000)]->Set_Render_All_Part(false);
	...
	//	m_Actors[static_cast<_uint>(CF150_ACTOR_TYPE::_SM41_024)]->Set_Render_All_Part(true);
}

HRESULT CCut_Scene_CF150::Add_Actors()
{
	m_Actors.resize(static_cast<size_t>(CF150_ACTOR_TYPE::_END));
	...
	if (FAILED(Add_Actor(TEXT("Prototype_GameObject_Actor_PL0000"), static_cast<_uint>(CF150_ACTOR_TYPE::_PL_0000), &Actor_PL0000_Desc)))
		return E_FAIL;*/
	...
	if (FAILED(Add_Actor(TEXT("Prototype_GameObject_Actor_SM41_024"), static_cast<_uint>(CF150_ACTOR_TYPE::_SM41_024), &Actor_SM41_024_Desc)))
		return E_FAIL;*/

	return S_OK;
}

HRESULT CCut_Scene_CF150::Add_Props()
{
	m_PropControllers.resize(static_cast<size_t>(CF150_PROP_TYPE::_END));

	/*CProp_Controller::PROP_CONTROLL_DESC			Prop_SM41_024_Desc;
	Prop_SM41_024_Desc.strAnimLayerTag = TEXT("CF150_SM41_024");

	if (FAILED(Add_PropController(TEXT("Prototype_GameObject_Prop_SM41_024"), static_cast<_uint>(CF150_PROP_TYPE::_SM41_024), &Prop_SM41_024_Desc)))
		return E_FAIL;*/

	return S_OK;
}

HRESULT CCut_Scene_CF150::Add_Camera_Event()
{
	m_strCamera_Event_Tag = TEXT("cf150");

	m_pEvent_Camera = (CCamera_Event*)m_pGameInstance->Get_GameObject(g_Level, g_strCameraTag, 1);
	if (nullptr == m_pEvent_Camera)
		return E_FAIL;

	if (FAILED(m_pEvent_Camera->Add_CamList(m_strCamera_Event_Tag, TEXT("../Bin/DataFiles/mcamlist/cf150.mcamlist.13"))))
		return E_FAIL;
...
CCut_Scene_CF150* CCut_Scene_CF150::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CCut_Scene_CF150* pInstance = new CCut_Scene_CF150(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CCut_Scene_CF150"));
...
CGameObject* CCut_Scene_CF150::Clone(void* pArg)
{
	CCut_Scene_CF150* pInstance = new CCut_Scene_CF150(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CCut_Scene_CF150"));
