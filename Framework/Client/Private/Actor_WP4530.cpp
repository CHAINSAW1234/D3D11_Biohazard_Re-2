...
void CActor_WP4530::Priority_Tick(_float fTimeDelta)
{
	...
	_matrix			WorldMatrix = { CCall_Center::Get_Instance()->Get_Caller(CCall_Center::CALLER::_PL00)->Get_Transform()->Get_WorldMatrix() };
...
void CActor_WP4530::Render_Light()
{
	...
	_matrix					ModelTransformationMtarix = { XMLoadFloat4x4(&pModel->Get_TransformationMatrix()) };
	...
	_matrix					ScaleMatrix = XMMatrixScalingFromVector(XMVectorSet(0.01f, 0.01f, 0.01f, 0.f));

	_matrix					ResultMatrix = { ResultCombiendMatrix * ScaleMatrix };

	_float4x4 fMatrix;
	XMStoreFloat4x4(&fMatrix, ResultMatrix);
	_float4 vPos = _float4(fMatrix._41, fMatrix._42, fMatrix._43, 1.f);


	const LIGHT_DESC* eDesc = m_pGameInstance->Get_LightDesc(TEXT("Light_Flash"), 0);
	...
	m_pGameInstance->Add_ShadowLight(CPipeLine::SPOT, TEXT("Light_Flash"));
}
