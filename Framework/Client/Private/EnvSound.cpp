...
{
	m_pGameInstance->BGMVolume_2D(m_fCurSound, 612);
	
	Set_SoundRegion(m_pPlayer->Get_Player_Region());
	_bool bPlaying = { false };
	m_pGameInstance->IsPlaying_2D(612, &bPlaying);
	if (m_fRegionChangeTimeDelay > 0.f)
	{
		...
		//m_pGameInstance->PlayBGM_2D(m_BGMSoundMap[m_iPreRegion], 612);
		if (!bPlaying)
			m_pGameInstance->PlaySoundEffect_2D(TEXT("location"), m_BGMSoundMap[m_iPreRegion], 612, m_fCurSound);
	}
...
HRESULT CEnvSound::Add_Components()
{
	...
	CloseHandle(hSoundLayerFile);
	//
	//HANDLE		hSoundFile = CreateFile(TEXT("../Bin/Data/Level_InteractObj/Env_Sound.dat"), GENERIC_READ, NULL, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
