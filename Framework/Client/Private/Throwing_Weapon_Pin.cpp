...
CThrowing_Weapon_Pin::CThrowing_Weapon_Pin(const CThrowing_Weapon_Pin& rhs)
	: CGameObject{ rhs }
{
}

HRESULT CThrowing_Weapon_Pin::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CThrowing_Weapon_Pin::Initialize(void* pArg)
{
	if (pArg == nullptr)
		return E_FAIL;
...
HRESULT CThrowing_Weapon_Pin::Render()
{
...
HRESULT CThrowing_Weapon_Pin::Render_LightDepth_Dir()
{
...
HRESULT CThrowing_Weapon_Pin::Render_LightDepth_Point()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
HRESULT CThrowing_Weapon_Pin::Render_LightDepth_Spot()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
HRESULT CThrowing_Weapon_Pin::Bind_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CThrowing_Weapon_Pin* CThrowing_Weapon_Pin::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CThrowing_Weapon_Pin* pInstance = new CThrowing_Weapon_Pin(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CThrowing_Weapon_Pin"));
...
CGameObject* CThrowing_Weapon_Pin::Clone(void* pArg)
{
	CThrowing_Weapon_Pin* pInstance = new CThrowing_Weapon_Pin(*this);
...
void CThrowing_Weapon_Pin::Free()
{
	__super::Free();
