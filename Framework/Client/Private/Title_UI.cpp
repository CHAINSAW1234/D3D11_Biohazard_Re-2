...
void CTitle_UI::TitleGame_Start()
{
    ...
    m_pGameInstance->PlaySoundEffect_2D(TEXT("UI"), TEXT("ui_title_media.bnk.2_30.mp3"), 0.5f);
    ...
    if(true == IsMouseHover())
    {
        if(TEXT("스토리") == m_vecTextBoxes.back()->Get_Text())
        {
            if(m_pGameInstance->Get_KeyState(VK_LBUTTON))
            {
                ...
                m_pGameInstance->PlaySoundEffect_2D(TEXT("UI"), TEXT("ui_title_media.bnk.2_21.mp3"), 0.5f);
        ...
        //m_pGameInstance->Change_Sound_2D(CHANNELID::CH30, TEXT("ui_ingame_media.bnk.2_18.mp3"));
        m_pGameInstance->PlaySoundEffect_2D(TEXT("UI"), TEXT("ui_ingame_media.bnk.2_18.mp3"), 0.5f);
