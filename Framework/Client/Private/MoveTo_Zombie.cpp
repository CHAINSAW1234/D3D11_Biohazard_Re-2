...
void CMoveTo_Zombie::Change_Animation()
{
	...
	_int			iCurrentPlayingBlendAnimIndex = { pBodyModel->Get_AnimIndex_PlayingInfo(CBody_Zombie::INDEX_1) };
	...
	{
		{
			if (true == isMoveAnim || true == isTurnAnim)
			{
				_bool		isFinished = { false };
				//	아직 첫 모션 중이었을경우 => 상태 체크가 필요함 => 스타트모션의 경우 끝나야 루프로 넘어가야함
				if (true == isStartAnim || true == isTurnAnim)
				{
					//	아직 스타트 모션 진행중
					if (false == pBodyModel->isFinished(iBasePlayingIndex))
					{
	...
	//	회전과 관련하여 블렌드 성분 정하기
	//	10도 미만은 그냥직진하기.
	if (XMConvertToRadians(10.f) > fAngleToTarget || XMConvertToRadians(90.f) < fAngleToTarget)
	{
		isNeedBlend = false;
	}
	...
	else
	{
		...
		fTurnBlendWeight = fminf(fAngleToTarget / XMConvertToRadians(90.f), 1.f);
	}
	...
	if (true == isNeedBlend)
	{
		//	pBodyModel->Change_Animation(iBasePlayingIndex, iBlendAnimIndex);
