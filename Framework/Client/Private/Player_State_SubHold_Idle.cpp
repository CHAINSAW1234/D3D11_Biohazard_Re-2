...
void CPlayer_State_SubHold_Idle::Shot()
{
	if (m_pGameInstance->Get_KeyState(VK_LBUTTON) == DOWN) {
		if (m_pPlayer->Get_Body_Model()->Is_Loop_PlayingInfo(3)) {
			m_pPlayer->Get_Body_Model()->Set_Loop(3, false);
			m_pPlayer->Get_Body_Model()->Set_BlendWeight(3, 10.f, 20.f);
			...
			if (fAngle >= 0) {
				m_pPlayer->Change_Body_Animation_Hold(3, CPlayer::HOLD_SHOT);
			}
			else {
				m_pPlayer->Change_Body_Animation_Hold(3, CPlayer::HOLD_SHOT_NO_AMMO);
			}
