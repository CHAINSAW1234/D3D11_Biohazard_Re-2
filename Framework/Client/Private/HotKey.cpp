...
void CHotKey::Start()
{
	for (_uint i = 0; i < EQ_SLOT_COUNT; i++)
	{
		...
		m_pEQItemUI[i]->Set_ItemUI(ITEM_NUMBER_END, HOTKEY, XMVectorSet(
			m_fPositions[i].x * static_cast<_float>(g_iWinSizeX) / 1600.f, 
			m_fPositions[i].y * static_cast<_float>(g_iWinSizeY) / 900.f
			, Z_POS_ITEM_UI,
			1.f), 0);
	}
