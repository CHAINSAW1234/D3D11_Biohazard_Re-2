...
#pragma region 1
	if (dwDirection & DIRECTION_FRONT) {
		m_pPlayer->Get_Body_Model()->Change_Animation(0, TEXT("Default"), CPlayer::STRAFEL_F);
		...
		if (dwDirection & DIRECTION_LEFT) {
			m_pPlayer->Get_Body_Model()->Change_Animation(1, TEXT("Default"), CPlayer::STRAFEL_L);
		}
		else if (dwDirection & DIRECTION_RIGHT) {
			m_pPlayer->Get_Body_Model()->Change_Animation(1, TEXT("Default"), CPlayer::STRAFEL_R);
		}
		else {
		...
		m_pPlayer->Get_Body_Model()->Change_Animation(0, TEXT("Default"), CPlayer::STRAFEL_B);
		m_pPlayer->Get_Body_Model()->Set_BlendWeight(0, .5f, 0.2f);
		m_pPlayer->Get_Body_Model()->Set_BlendWeight(1, .5f, 0.2f);
		...
		if (dwDirection & DIRECTION_LEFT) {
			m_pPlayer->Get_Body_Model()->Change_Animation(0, TEXT("Default"), CPlayer::STRAFEL_L);
		}
		else if (dwDirection & DIRECTION_RIGHT) {
			m_pPlayer->Get_Body_Model()->Change_Animation(0, TEXT("Default"), CPlayer::STRAFEL_R);
		}
		else {
...
void CPlayer_State_Hold_Idle::Shot()
{
	...
	if (m_pGameInstance->Get_KeyState(VK_LBUTTON) == PRESSING && m_pPlayer->Get_Body_Model()->isFinished(3)) {
		if (!m_isShot && m_pPlayer->Get_Body_Model()->Get_BlendWeight(2) == 0.f) {
			...
			if (1) {
				// 총알 있으면
				m_pPlayer->Get_Body_Model()->Change_Animation(2, TEXT("Default"), CPlayer::HOLD_SHOT);
				// 총알 발사하는 함수 추가
			}
			else {
				// 총알 없음
				m_pPlayer->Get_Body_Model()->Change_Animation(2, TEXT("Default"), CPlayer::HOLD_SHOT_NO_AMMO);
			}
