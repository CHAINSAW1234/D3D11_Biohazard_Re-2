...
private:
	...
	void Puszzle_button_Sound(_int iButton);
...
private:
	...
	_int				m_iSelected_Button = { 0 };
	_uint				m_iPuzzle_Progress = 0;
	_bool				m_isFirstPress = { false };

	string				m_LampTags[8];
	string				m_ButtonTags[8];
	string				m_ButtonBoneTags[8];

	_int				m_iInputAnswer[8] = { -1, -1, -1, -1, -1, -1, -1, -1 };

	_uint				m_iSetByNumAnswer[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
	_uint				m_iAnswer[8] = { 2, 1, 5, 0, 6, 3, 7, 4 };
